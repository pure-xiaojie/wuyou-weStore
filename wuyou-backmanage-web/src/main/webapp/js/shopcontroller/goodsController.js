 //控制层 
app.controller('goodsController' ,function($scope,$controller,$location,orderService   ,goodsService,uploadService,itemCatService,typeTemplateService){	
	...
	$scope.findAll=function(){
		goodsService.findAll().success(
			function(response){
				$scope.list=response;
				$scope.sum = $scope.list.length;
				for(var i = 0; i < $scope.list.length; i++) {
					$scope.num[$scope.list[i].id] = 0;
				}
			}			
		);
	...
	findOrderList=function(){
		...
		orderService.findOrderList().success(
			function(response){
				...
				$scope.size=$scope.orderList.length;
				for(var i = 0; i < $scope.orderList.length; i++) {
					$scope.paySum+=$scope.orderList[i].payment;
	...
	getList=function(list) {
		var j = 0;
		for(var i=0; i < list.length; i++) {
			if($scope.num[$scope.list[i].id] != 0) {
				$scope.numList[j]=$scope.list[i];
				j++;
			}
